...
/**
 * This is the single (non-parallel) monitoring task which takes a {@link FileInputFormat} and, depending on the {@link
 * FileProcessingMode} and the {@link FilePathFilter}, it is responsible for:
 *
 * <ol> <li>Monitoring a user-provided path.</li> <li>Deciding which files should be further read and processed.</li>
 * <li>Creating the {@link FileInputSplit splits} corresponding to those files.</li> <li>Assigning them to downstream
 * tasks for further processing.</li> </ol>
 *
 * <p>The splits to be read are forwarded to the downstream {@link ContinuousFileReaderOperator} which can have
 * parallelism greater than one.
 *
 * <p><b>IMPORTANT NOTE: </b> Splits are forwarded downstream for reading in ascending modification time order, based on
 * the modification time of the files they belong to.
 */
@Internal
public class ContinuousFileMonitoringFunction<OUT> extends RichSourceFunction<TimestampedFileInputSplit> implements CheckpointedFunction {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(org.apache.flink.streaming.api.functions.source.ContinuousFileMonitoringFunction.class);
  ...
  @Override
  public void run(SourceFunction.SourceContext<TimestampedFileInputSplit> context) throws
      Exception {
    Path p = new Path(path);
    FileSystem fileSystem = FileSystem.get(p.toUri());

    //
    // THIS CHECK DOESN'T WORK WITH THE GCS CONNECTOR ON GCLOUD...
    //
  ...
  /**
   * Creates the input splits to be forwarded to the downstream tasks of the {@link ContinuousFileReaderOperator}.
   * Splits are sorted <b>by modification time</b> before being forwarded and only splits belonging to files in the
   * {@code eligibleFiles} list will be processed.
   *
  ...
  private Map<Long, List<TimestampedFileInputSplit>> getInputSplitsSortedByModTime(
    ...
    // Note: modified to 1 instead of readerParallelism in order to not split files up...
    FileInputSplit[] inputSplits = format.createInputSplits(1);

    getRuntimeContext().getIntCounter("SplitReaderNumSplits").add(inputSplits.length);
   ...
   */
  private Map<Path, FileStatus> listEligibleFiles(FileSystem fileSystem, Path path) throws
      IOException {
  ...
  /**
   * Returns {@code true} if the file is NOT to be processed further. This happens if the modification time of the file
   * is smaller than the {@link #globalModificationTime}.
   *
